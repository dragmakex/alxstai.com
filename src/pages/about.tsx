        ...
        <section className="min-h-screen"> 
          ...
          <canvas id='blur' className='fixed top-0 left-0 w-full h-full z-[-9]'>
            ...
            <div className='2xl:py-6'>
              <div className={`inset-0 border-8 border-gray-400 dark:border-white flex items-center relative mx-auto rounded-full w-60 h-60 md:w-90 md:h-90 2xl:h-120 2xl:w-120 mt-10 mb-10 overflow-hidden dark:bg-gray-400 bg-black transition-all duration-700 ${loaded ? 'opacity-100 translate-y-10' : 'opacity-0 translate-y-0'}`}>
                  <Image src={sticky_luffy} fill objectFit="cover" alt={'Picture'} className='border-lg border-white'/>
              </div>
          
              <div className={`text-4xl md:text-5xl flex justify-center gap-10 md:gap-16 lg:gap-20 py-4 transition-all duration-700 ${loaded ? 'opacity-100 translate-y-10' : 'opacity-0 translate-y-0'}`}>
