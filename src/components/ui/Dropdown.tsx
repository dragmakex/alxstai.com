...
import droplist from "../../data/navigation.json"
                    ...
                    <div className="md:w-[120px] absolute flex flex-col top-10 items-center rounded-lg p-2"> 
                        {droplist.map((item, i) => (
                                <Link href={item.path} key={i}>
                                    <div className="font-tisa items-center flex w-full justify-center p-1 hover:bg-gradient-to-r from-purple-300 via-pink-300 to-indigo-400 dark:from-purple-400 dark:via-pink-400 dark:to-indigo-500 cursor-pointer rounded-r-md border-l-transparent hover:border-l-black hover:dark:border-l-gray-400 border-l-4 active:text-white duration-200" key={i}>
                                        <h3>{item.tab}</h3>
                                    </div>
                                </Link>
                            )
